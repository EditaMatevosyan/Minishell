...
/*   Updated: 2025/12/20 19:20:44 by edmatevo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int					handle_operator_token(t_minishell *shell, char *input,
						int *i);
...
int					set_env_value(t_env **env, const char *key,
						const char *val);
int					append_env_value(t_env **env, const char *key,
						const char *suffix);
...
char				*join_expanded_arg(t_token **tok, t_env *env,
						int *skip_arg);
int					open_infile(t_cmd *cmd);
int					open_outfile(t_cmd *cmd);
int					change_stdin(t_cmd *cmd);
int					change_stdout(t_cmd *cmd);
int					ft_strcpy_without_null_termination(char *dst,
						const char *src);
...
void				validate_and_exec(t_cmd *cmd, t_minishell *shell,
						char **envp_array);
...
void				exec_stat_or_notfound(t_exec_ctx *ctx, char *path,
						int has_slash, struct stat *st);
...
void				restore_std_fds(t_minishell *shell, int saved_stdin,
						int saved_stdout);
void				handle_builtin_redir_failure(t_minishell *shell,
						int saved_stdin, int saved_stdout);
...
void				init_exec_ctx_2(t_exec_ctx_2 *ctx, t_cmd *cmd_list,
						t_minishell *ms);
