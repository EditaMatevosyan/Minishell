...
static int	is_invalid_pair(t_token *tok)
{
	if (tok->type == T_PIPE && tok->next->type == T_PIPE)
		return (print_syntax_error("|"), 1);
	if (tok->type != T_PIPE && operator_type(tok->type)
		&& operator_type(tok->next->type))
		return (print_syntax_error(tok->next->value), 1);
	return (0);
}
