...
/*   Updated: 2025/10/02 17:28:58 by edmatevo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int tokenize_input(t_minishell *shell, char *input)
{
    ...
    while (input[i])
    {
        ...
        if (!input[i])
            break;
        if (is_operator(input[i]) && handle_operator_token(shell, input, &i))
            return -1;
        else if (is_quote(input[i]) && handle_quote_token(shell, input, &i))
            return -1;
        else if (handle_word_token(shell, input, &i))
            return -1;
    }
    return 0;
}
